...
function FooterInfoModal({ isOpen, onClose, title, content }: { isOpen: boolean; onClose: () => void; title: string; content: string }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
      <div className="bg-gray-900 rounded-2xl max-w-2xl w-full h-[70vh] flex flex-col border border-white/10">
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          {currentStep === 1 && (
            <div className="bg-white/5 rounded-2xl p-8 border border-white/10">
              ...
              {/* CAPTCHA Widget - Android WebView Optimized */}
                  ...
                  {DAYS_OF_WEEK.map(day => {
                    ...
                    return (
                      <div key={day} className="border border-white/10 rounded-xl overflow-hidden">
                        <button onClick={() => toggleDay(day)} className="w-full flex items-center justify-between px-4 py-3 bg-white/5 hover:bg-white/10">
                          <div className="flex items-center gap-3">
                            {isExpanded ? <ChevronDown className="w-4 h-4 text-gray-400" /> : <ChevronRight className="w-4 h-4 text-gray-400" />}
                            <span className="font-medium">{day}</span>
                          </div>
                          <span className={`text-xs px-2 py-0.5 rounded-full ${dayAvail.times.length > 0 ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'}`}>
                            {dayAvail.times.length > 0 ? `${dayAvail.times.length} slot${dayAvail.times.length > 1 ? 's' : ''}` : 'No slots'}
                          </span>
                        </button>
                        {isExpanded && (
                          <div className="px-4 py-3 bg-black">
                            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
                              {generateTimeSlots(true).map(time => (
                                <button key={time} onClick={() => toggleTimeSlot(day, time)} className={`px-2 py-1.5 rounded-lg text-xs font-medium transition-all ${dayAvail.times.includes(time) ? 'bg-red-600 text-white' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}>
                                  {formatTimeLabel(time)}
                                </button>
                              ))}
                            </div>
                            {dayAvail.times.length > 0 && (
                              <button onClick={() => setAvailability(prev => prev.map(a => a.day === day ? { ...a, times: [] } : a))} className="mt-3 text-xs text-red-400 hover:text-red-300">
            ...
            <div className="bg-white/5 rounded-2xl p-8 border border-white/10">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mb-2">How far in advance must members book a session?</p>
                  <select value={minAdvanceNotice} onChange={(e) => setMinAdvanceNotice(parseInt(e.target.value))} className="w-full px-4 py-3 bg-black border border-white/20 rounded-xl text-white focus:border-red-500 focus:outline-none">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mb-2">How many hours before a session can members cancel without penalty?</p>
                  <select value={cancellationWindow} onChange={(e) => setCancellationWindow(parseInt(e.target.value))} className="w-full px-4 py-3 bg-black border border-white/20 rounded-xl text-white focus:border-red-500 focus:outline-none">
                ...
                <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-xl">
                  <p className="text-xs text-blue-300 text-center">💡 Tip: Setting longer notice periods helps you manage your schedule better. Your time is valuable — protect it.</p>
                </div>
                
                <div className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-xl">
                  <p className="text-xs text-yellow-300 text-center">⚠️ Please review all sections before finalizing.</p>
                </div>
              </div>
              
              <div className="flex justify-between gap-4 mt-8">
                <button onClick={handleBack} className="flex-1 py-3 bg-white/10 hover:bg-white/20 rounded-xl font-semibold transition">← BACK</button>
                <button onClick={handleNext} disabled={saving} className="flex-1 py-3 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-700 hover:to-orange-700 rounded-xl font-semibold transition hover:scale-105 disabled:opacity-50">
