...
- You have the right to opt out of data sharing under CPRA`;
  ...
  const handleNext = async () => {
    if (currentStep === 1) {
      // Specific error collection for Step 1
      const errors = [];
      if (!firstName) errors.push('First name is required');
      if (!lastName) errors.push('Last name is required');
      if (!email) errors.push('Email is required');
      if (!phone) errors.push('Phone number is required');
      if (!isPasswordValid) errors.push('Password does not meet requirements');
      ...
      if (!gatekeeperAccepted) errors.push('You must acknowledge the platform guidelines');
      ...
      if (usernameAvailable !== true) errors.push('Username is not available');
      if (!city) errors.push('City is required');
      if (!emergencyName) errors.push('Emergency contact name is required');
      if (!emergencyPhone) errors.push('Emergency phone number is required');
      if (!emergencyRelationship) errors.push('Emergency contact relationship is required');
      if (!emergencyConfirmed) errors.push('You must confirm your emergency contact information');
      if (!affirmNoSexOffender) errors.push('You must affirm you are not a registered sex offender');
      if (!affirmNoViolentFelony) errors.push('You must affirm you have no violent felony convictions');
      ...
      if (!isStep3Complete()) {
        const errors = [];
        if (allPhotos.length === 0) errors.push('At least one photo is required');
        if (!photoConfirmed) errors.push('You must confirm your photo meets the guidelines');
        setStep3Error(errors.join('. '));
        return;
      }
      setStep3Error('');
      setCurrentStep(4);
      ...
      if (!isStep4Complete()) {
        const errors = [];
        if (allSelectedServices.length === 0) errors.push('Select at least one activity');
        const invalidServices = allSelectedServices.filter(service => {
          const r = serviceRates[service];
          if (!r || !r.hourly || r.hourly < 50 || r.hourly > 500) return true;
          if (serviceHalfHourEnabled[service] && (!r.halfHour || r.halfHour < 30 || r.halfHour > r.hourly)) return true;
          return false;
        });
        if (invalidServices.length > 0) errors.push(`Invalid rates for: ${invalidServices.join(', ')}`);
        setStep4Error(errors.join('. '));
        return;
      }
      setStep4Error('');
      if (user) {
      ...
      if (!isStep5Complete()) {
        const errors = [];
        if (serviceAreas.length === 0) errors.push('Add at least one location');
        if (!availability.some(a => a.times.length > 0)) errors.push('Select at least one availability slot');
        setStep5Error(errors.join('. '));
        return;
      }
      setStep5Error('');
      if (user) {
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    <span className="text-sm text-gray-300">I have read and agree to the <button type="button" onClick={() => setShowTermsModal('terms')} className="text-red-400 underline">Terms of Service</button>. <span className="text-red-500">*</span></span>
                  </label>
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    <span className="text-sm text-gray-300">I have read and agree to the <button type="button" onClick={() => setShowTermsModal('privacy')} className="text-red-400 underline">Privacy Policy</button>. <span className="text-red-500">*</span></span>
                  </label>
                  <label className="flex items-start gap-3 cursor-pointer">
