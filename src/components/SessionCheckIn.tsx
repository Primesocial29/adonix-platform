...
export default function SessionCheckIn({ booking, onCheckInComplete }: SessionCheckInProps) {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      // Updated to 0.75 miles (1207 meters) to match Terms of Service
      if (distance > 1207) {
        throw new Error(
          `You are ${Math.round(distance)}m away. You must be within 0.75 miles (1207m) of ${booking.location_name} to check in.`
        );
      }
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 mb-4">
            Check in when you arrive at {booking.location_name}. GPS will verify you're within 0.75 miles (1207m) of the location.
          </p>
          
          {/* GPS Verification Notice */}
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3 flex gap-3 mb-4">
            <MapPin className="w-5 h-5 text-blue-400 flex-shrink-0 mt-0.5" />
            <div className="text-xs text-blue-200">
              <p className="font-medium mb-1">📍 GPS Verification Required</p>
              <p>
                Both parties must be within 0.75 miles (1207 meters) of the agreed location before payment is released. 
